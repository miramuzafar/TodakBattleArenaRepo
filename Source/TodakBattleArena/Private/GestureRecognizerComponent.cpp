...
void UGestureRecognizerComponent::InitializeComponent()
{
	...
	AGameModeBase* Mode = UGameplayStatics::GetGameMode(Owner);
	if (Mode)
	{
		UE_LOG(LogTemp, Warning, TEXT("AController name %s"), *Mode->GetName());
		PlayerControllerClass = Mode->PlayerControllerClass;
		//APlayerController* Controller = Cast<APlayerController>(PlayerControllerClass);
		if (PlayerControllerClass)
		{
			UE_LOG(LogTemp, Warning, TEXT("PlayerController name %s"), *PlayerControllerClass->GetName());
			APlayerController* Controller = PlayerControllerClass.GetDefaultObject();
			if (Controller)
			{
				UE_LOG(LogTemp, Warning, TEXT("APlayerController name %s"), *Controller->GetName());
